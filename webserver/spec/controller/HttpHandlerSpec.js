...
describe("try calling http server)", function() {
  beforeEach(function() {
    ...
    spyOn(concreteDataBaseHandler, 'getLimitedNrOfTemperatureValuesFromDB').and.callFake(function() {
      arguments[3](null, testTempData);
    });
    spyOn(concreteDataBaseHandler, 'getLimitedNrOfHumidityValuesFromDB').and.callFake(function() {
  ...
  it("init", function(done) {
    uut.runHTTPServer();
    expect(concreteDataBaseHandler.getLimitedNrOfTemperatureValuesFromDB).toHaveBeenCalledWith(testMac, testData.nrOfDatapoints, 'timestamp DESC', jasmine.any(Function));
    expect(socket.emit).toHaveBeenCalledWith('setLastTempDataByMac', testTempEmitData);
    expect(concreteDataBaseHandler.getLimitedNrOfHumidityValuesFromDB).toHaveBeenCalledWith(testMac, testData.nrOfDatapoints, 'timestamp DESC', jasmine.any(Function));
    expect(socket.emit).toHaveBeenCalledWith('setLastHumDataByMac', testHumEmitData);
    done();
  });
});
